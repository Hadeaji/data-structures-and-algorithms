...
def get_edge(de_graph, checkpoints):
    if de_graph.size == 'Graph is empty':
        return 'False'
    else:
        total =0
        checkpoint=0

        for f in range(0,len(checkpoints)-1):
            temp = total
            x= de_graph.get_neighbours(checkpoints[checkpoint])
            for i in x:
                if checkpoints[checkpoint+1] == i[1]:
                    total+= i[0]
            if temp == total:
                return False
            checkpoint +=1
